function [X, fX, i] = minimize_mtgp(X, f, len, X_test,y_test_ideal_1,x1_test_1,x2_test_1,covfunc,likfunc,X_train,y_train, varargin)

% Minimize a differentiable multivariate function using conjugate gradients.
%
% Usage: [X, fX, i] = minimize(X, f, len, P1, P2, P3, ... )
% 
...
% len  len of the run; if it is positive, it gives the maximum number of
%         line searches, if negative its absolute gives the maximum allowed
%         number of function evaluations. Optionally, len can have a second
...
%         depending on the sign of "len") used at termination.
%
% The function returns when either its len is up, or if no further progress
...
if max(size(len)) == 2, red=len(2); len=len(1); else red=1; end
if len>0, S='Linesearch'; else S='Function evaluation'; end 
...
m = MTGP(X,@MTGP_infExact,[],covfunc,likfunc,X_train,y_train,X_test);
m_1 = m(1:length(y_test_ideal_1));
PSNR_1 = max(y_test_ideal_1(:))*sqrt(length(x1_test_1)*length(x2_test_1))/sqrt(sum((m_1(:)-y_test_ideal_1(:)).^2));
PSNR_1 = 20*log(PSNR_1)/log(10);
SSIM_1 = ssim(m_1,y_test_ideal_1);

X = unwrap(X); df0 = unwrap(df0);



fprintf('%s %6i;  Value %4.6e;  PSNR %.4f;  SSIM %.4f\r', S, i, f0, PSNR_1, SSIM_1);
...
while i < abs(len)                                      % while not finished
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    m = MTGP(rewrap(Z, X),@MTGP_infExact,[],covfunc,likfunc,X_train,y_train,X_test);
    m_1 = m(1:length(y_test_ideal_1));
    PSNR_1 = max(y_test_ideal_1(:))*sqrt(length(x1_test_1)*length(x2_test_1))/sqrt(sum((m_1(:)-y_test_ideal_1(:)).^2));
    PSNR_1 = 20*log(PSNR_1)/log(10);
    SSIM_1 = ssim(m_1,y_test_ideal_1);
