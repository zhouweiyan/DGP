function [X, fX, i] = minimize_gp(X, f, len, X_test_1,y_test_ideal_1,x1_test_1,x2_test_1,covfunc,likfunc,X_train_1,y_train_1, varargin)
...
m_1 = gp(X,@infGaussLik,[],covfunc,likfunc,X_train_1(:,1:end-1),y_train_1,X_test_1(:,1:end-1));
...
while i < abs(len)                                      % while not finished
  ...
  if abs(d3) < -SIG*d0 && f3 < f0+x3*RHO*d0          % if line search succeeded
    ...
    m_1 = gp(rewrap(Z, X),@infGaussLik,[],covfunc,likfunc,X_train_1(:,1:end-1),y_train_1,X_test_1(:,1:end-1));
    PSNR_1 = max(y_test_ideal_1(:))*sqrt(length(x1_test_1)*length(x2_test_1))/sqrt(sum((m_1(:)-y_test_ideal_1(:)).^2));
    PSNR_1 = 20*log(PSNR_1)/log(10);
    SSIM_1 = ssim(m_1,y_test_ideal_1);
