function K = MTGP_covPPisoU(v, hyp, x, z, i)
...
%
% k(x^p,x^q) = max(1-r,0)^(j+v) * f(r,j) with j = floor(D/2)+v+1
%
...
if dg                                                               % vector kxx
  ...
  if xeqz                                                 % symmetric matrix Kxx
    K = sqrt( sq_dist(x(:,1:end-1)'/ell) );
  else                                                   % cross covariances Kxz
    K = sqrt( sq_dist(x(:,1:end-1)'/ell,z(:,1:end-1)'/ell) );
  end
end
